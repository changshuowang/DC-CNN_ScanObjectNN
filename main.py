...
#from models import DCCNN_MSN_Cls as DCCNN_SSN
...
def parse_args():
    ...
    parser.add_argument('--model', default='DC-CNN', help='model name [default: DC-CNN]')
    ...
    parser.add_argument('--wa', action='store_false', help='use weight average' )
    parser.add_argument('--wa_start', default=0.9, type=float, help='use weight average, when to start' )
    parser.add_argument('--num_kernel', default=4, type=int, help='the number of dynamic kernel')
...
def main():
    ...
    if args.msg is None:
        ...
        message = "-" + args.msg
    args.checkpoint = 'log/' + args.model + message
    ...
    net = DCCNN_SSN(num_classes = args.num_classes, num_kernel = args.num_kernel, input_channels = 0, relation_prior = 1, use_xyz = True)
    criterion = cal_loss
    net = net.to(device)
    printf(f"ScanObjectNN: {net}")
    ...
    if not os.path.isfile(os.path.join(args.checkpoint, "last_checkpoint.pth")):
        save_args(args)
        logger = Logger(os.path.join(args.checkpoint, 'log.txt'), title="ScanObjectNN" + args.model)
        ...
        logger = Logger(os.path.join(args.checkpoint, 'log.txt'), title="ScanObjectNN" + args.model, resume=True)
    ...
    wa_flag = args.wa
    wa_start = args.wa_start*args.epoch

    for epoch in range(start_epoch, args.epoch):
        printf('Epoch(%d/%s) Learning Rate %s:' % (epoch + 1, args.epoch, optimizer.param_groups[0]['lr']))
        
        if args.wa and wa_flag and epoch >= wa_start:
            wa_flag = False
            swa_model = swa_utils.AveragedModel(net)
            swa_utils.update_bn(train_loader, swa_model, device = device )
            printf('start weight avg')       
        
        train_out = train(net, train_loader, optimizer, criterion, device)  # {"loss", "acc", "acc_avg", "time"}
        test_out = validate(net, test_loader, criterion, device)
        scheduler.step()

        if args.wa and epoch >= wa_start:
            swa_model.update_parameters(net)
            if epoch == args.epoch-1 : # save last avg module
                torch.save(swa_model.module.state_dict(), args.checkpoint + '/net_average_nobn.pth')
                swa_utils.update_bn( train_loader, swa_model, device = device)
                torch.save(swa_model.module.state_dict(), args.checkpoint + '/net_average.pth')
