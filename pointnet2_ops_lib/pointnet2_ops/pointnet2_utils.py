...
try:
    import pointnet2_ops_lib.pointnet2_ops._ext as _ext
except ImportError:
...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz, new_xyz, features=None, fps_idx: torch.IntTensor = None):
        ...
        if features is not None:
            ...
            center_features = gather_operation(features, fps_idx)
            new_grouped_features = grouped_features - center_features.unsqueeze(-1)

            if self.use_xyz:
                new_features = torch.cat([raw_grouped_xyz, grouped_xyz, new_grouped_features],dim=1)  # (B, C + 3 + 3, npoint, nsample)
            else:
                new_features = grouped_features
        else:
            assert self.use_xyz, "Cannot have not features and not use xyz as a feature!"
            new_features = torch.cat([raw_grouped_xyz, grouped_xyz], dim = 1)
            # print("0_features", new_features.shape)
